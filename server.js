...
const POLL_MS = Math.max(1500, Number(process.env.POLL_MS || 2000));
const REQUEST_TIMEOUT_MS = Math.max(5000, Number(process.env.REQUEST_TIMEOUT_MS || 15000));
...
const SOURCE_TARGET_MS = Math.max(500, Number(process.env.SOURCE_TARGET_MS || 5000));
const MAX_MARKET_ROWS = Math.min(60000, Math.max(100, Number(process.env.MAX_MARKET_ROWS || 60000)));
...
// Keep long-lived market/API connections stable under heavier traffic.
server.keepAliveTimeout = Math.max(10000, Number(process.env.KEEP_ALIVE_MS || 15000));
server.headersTimeout = Math.max(
  server.keepAliveTimeout + 5000,
  REQUEST_TIMEOUT_MS + 7000
);
server.requestTimeout = Math.max(20000, REQUEST_TIMEOUT_MS + 5000);
server.maxRequestsPerSocket = Math.max(1000, Number(process.env.MAX_REQUESTS_PER_SOCKET || 10000));
...
app.use((req,res,next)=>{
  ...
  if (origin) {
    ...
    res.set("Access-Control-Allow-Headers", "Content-Type, X-Auth-Key, X-Request-ID");
...
async function httpJson(url, headers = {}, timeout = REQUEST_TIMEOUT_MS, options = {}) {
  let last;
  const attempts = Math.max(2, Number(options.attempts || 4));
  const baseDelay = Math.max(150, Number(options.retryDelayMs || 250));
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    try {
      const res = await fetch(url, {
        signal: ctrl.signal,
        headers: {
          "User-Agent": `IranMarketLive/${VERSION}`,
      ...
      try {
        ...
        throw new Error(/مسدود|دسترسی شما|General Error/i.test(text)
          ? "source blocked"
          : "non-json response");
      }
    } catch (e) {
      ...
      if (attempt < attempts - 1 && retryable) {
        ...
        await sleep(Math.min(5000, baseDelay * (2 ** attempt) + jitter));
      }
    } finally {
      clearTimeout(timer);
    }
...
async function fetchCdn() {
  const qs = "industrialGroup=&paperTypes[0]=1&paperTypes[1]=2&paperTypes[2]=3&paperTypes[3]=4&paperTypes[4]=5&paperTypes[5]=6&paperTypes[6]=7&paperTypes[7]=8&paperTypes[8]=9&showTraded=true&withBestLimits=false&hEven=0&RefID=0";
  const urls = [
    `${CDN}/api/ClosingPrice/GetMarketWatch?market=0&${qs}`,
    `${CDN}/api/ClosingPrice/GetMarketWatch?market=1&${qs}`,
    `${CDN}/api/ClosingPrice/GetMarketWatch?market=2&${qs}`,
    `${CDN}/api/ClosingPrice/GetMarketWatch?market=4&${qs}`
  ];
  // Fetch every market bucket and merge them. The old implementation stopped
  // after the first healthy bucket, which could silently cap the live universe.
  const settled = await Promise.allSettled(urls.map(u => httpJson(u)));
  const byInsCode = new Map();
  let successfulBuckets = 0;
  for (const r of settled) {
    if (r.status !== "fulfilled") continue;
    const candidate = normalizeMarketPayload(r.value);
    if (candidate.length) successfulBuckets++;
    for (const row of candidate) {
      const id = String(row.insCode || "");
      if (!id) continue;
      const prev = byInsCode.get(id);
      // Prefer the row with more useful live fields.
      const score = x => (x.last ? 2 : 0) + (x.close ? 2 : 0) + (x.volume ? 1 : 0) + (x.value ? 1 : 0);
      if (!prev || score(row) > score(prev)) byInsCode.set(id, row);
    }
  }
  let market = [...byInsCode.values()];
  if (market.length < SOURCE_MIN_ROWS)
    throw new Error(`CDN marketwatch returned ${market.length} usable rows across ${urls.length} buckets`);

  // Client-type data is enrichment only: a failure must NOT kill live prices.
  let client = null;
  try { client = await cdn("/api/ClientType/GetClientTypeAll", 4500); } catch {}
  return client ? mergeClientType(market, client) : market;
}
...
async function fetchMarket(){
  ...
  for(let i=0;i<results.length;i++){
    ...
    const rows=r.value.data.map(enrich).slice(0, MAX_MARKET_ROWS);
...
app.get("/api/ping", (req,res)=>{
  ...
  res.json({ok:true,service:"iran-market-live",version:VERSION,time:new Date().toISOString(),host:req.hostname,requestId:req.requestId});
});

app.get("/api/config", (req,res)=>{
  res.set("Cache-Control","no-store");
  res.json({version:VERSION,wsPath:"/ws",apiPath:"/api",serverTime:new Date().toISOString(),authRequired:Boolean(AUTH_KEY)});
});

app.get("/api/market", guard, (req, res) => {
  res.set("Cache-Control", "no-store");
  const requested = Number.parseInt(req.query.limit, 10);
  const limit = Number.isFinite(requested) ? Math.min(MAX_MARKET_ROWS, Math.max(1, requested)) : MAX_MARKET_ROWS;
  res.json({ rows: state.rows.slice(0, limit), meta: {
    source: state.source, updatedAt: state.updatedAt,
    latencyMs: state.latencyMs, error: state.error, cycle: state.cycle,
    stale: state.stale, sourceEvidence: state.sourceEvidence,
    totalRows: state.rows.length, returnedRows: Math.min(limit, state.rows.length),
    maxRows: MAX_MARKET_ROWS
  }});
});
...
server.listen(PORT, HOST, () => {
  console.log(`Iran Market Live Pro: http://${HOST}:${PORT}`);
